...
fun SurfaceTypeChoiceScreen(
    ...
    Column(modifier = Modifier.padding(all = 16.dp)) {
        ...
        Text(
            ...
            text = stringResource(id = R.string.chooseSurfaceType),
            style = S20_W700,
            color = DarkBlue,
        ...
        Spacer(modifier = Modifier.height(20.dp))
        LazyColumn {
            items(SurfaceType.entries) { surfaceType ->
                Column(
                    modifier = Modifier
                        .fillMaxWidth()
                        .clickable {
                            viewModel.onSurfaceTypeChange(surfaceType)
                            onNavigateBack()
                        }
                ) {
                    Text(
